    ...
    for i in host_validos:
        try:
            ...
            for proto in nm[i].all_protocols():
                ...
                print(f"Uso da memória: {round(uso_memoria[0]/(1024*1024*1024),2)}GB")
                disco = psutil.disk_usage('/')
                disco_total = disco[0]
                disco_ocupado = disco[1]
                disco_livre = disco[2]
                print(f"O total de disco do sistema é: {round(disco_total/(1024*1024*1024),2)}GB")
                print(f"O total de disco ocupado é {round(disco_ocupado/(1024*1024*1024),2)}GB")
                print(f"O total de disco livre é {round(disco_livre/(1024*1024*1024),2)}GB")
